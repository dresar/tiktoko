...
class Themedd_Updater_Admin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args( $config, array(
			'remote_api_url' => 'https://tematoko.com',
	...
	function updater() {
		...
		if ( ! class_exists( 'Themedd_Updater' ) ) {
	...
	function license_page() {
		...
		<div class="wrap license-wrap">
			<h2 class="headline"><?php echo sprintf( __( '%s License', 'themedd' ), TIKSTORE_NAME ); ?></h2>
			<div class="themedd-license-management-wrap">
				<h2 class="themedd-license-management-headline"><?php echo sprintf( __( 'Activate Your %s License Key', 'themedd' ), TIKSTORE_NAME ); ?></h2>
				<p>
					<?php echo sprintf( __( 'Your license key grants you access to theme updates and support. If your license key is deactivated or expired, your theme will work properly but you will not receive automatic updates.', 'themedd' ) );
					?>
				</p>
				<h3><strong><?php _e( 'License activation instructions', 'themedd' ); ?></strong></h3>
				<ol class="themedd-license-instructions">
					<li><?php _e( 'Enter your license key.', 'themedd' ); ?></li>
					<li><?php _e( 'Click the "Save License Key Changes" button.', 'themedd' ); ?></li>
					<li><?php _e( 'Click the new "Activate License" button.', 'themedd' ); ?></li>
					<li><?php _e( 'You\'re done! The status of your license displays below the License Key field.', 'themedd' ); ?></li>
				</ol>
				<form method="post" action="options.php">
					<?php settings_fields( $this->theme_slug . '-license' ); ?>
					<h3 class="license-key-label"><?php echo $strings['license-key']; ?></h3>
					<div>
						<input id="<?php echo $this->theme_slug; ?>_license_key" name="<?php echo $this->theme_slug; ?>_license_key" type="text" class="regular-text" value="<?php echo $this->get_hidden_license( $license ); ?>" />
					</div>
					<p class="description">
						<?php echo $message; ?>
					</p>
	...
	function get_hidden_license() {

		$status = get_option( 'tiktoko_license_key_status', false );
		if ( 'valid' == $status ) {
			$input_string = get_option( 'tiktoko_license_key' );
		}
